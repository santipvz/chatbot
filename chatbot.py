...
def addSymptons():
    ...
    answer = input2(
        f"Escriba los síntomas que tenga o pulse enter para salir\nComo por ejemplo: {randomSymptom}\n"
    ).lower()
    ...
    while answer != "" and any(x == 0 for x in userSymptoms):
        ...
        for v in answer.split():
            if v in restricted_words:
                input2(
                    "Intente no hacer negaciones a la hora de introducir los síntomas.\nPulse enter para continuar..."
                )
                break
            ...
            if len(symptomsAdded) == 0:
                option = input2(
                    "No sé detectó ningún síntoma no nombrado anteriormente\n¿Quiere ver todos los posibles síntomas para añadir? [Y] "
                )
                if option in ["y", "Y"]:
                    addableSymptons()
    ...
    if len(newpercentages) <= 0:
        option = input2(
            "Ninguna enfermedad se corresponde con los síntomas añadidos\n\
...
if __name__ == "__main__":
    ...
    print2(
        "A continuación le haremos unas preguntas para hacer una evalución de las posibles enfermedades que podría presentar."
    )
    ...
    while not flag:
        ...
        if evolucion in ["no", "No", "no.", "No."]:
            print2(
                "Si sus síntomas son constantes y no cesan se le dará una cita médica para después de esta consulta."
            )
            flag = True
        if not flag:
    ...
    hereditario = input2(
        "¿Algún familiar suyo ha sido diagnosticado con alguna  enfermedad abdominal?\nEn caso afirmativo, escríbala. De lo contrario pulse enter.\n"
    )
    while hereditario != "":
        if (
            any((hereditario.capitalize() == x[:-1]) for x in enfermedades)
            and hereditario.capitalize() not in enfermedadesHereditarias
        ):
            enfermedadesHereditarias.append(hereditario.capitalize())
        else:
            print2("Enfermedad no reconocida por nuestra base de datos.")
        hereditario = input2(
            "Introduzca otra enfermedad o pulse enter para continuar\n"
        )
    ...
    ubi = input(
        """Indique la zona del dolor o pulse enter para salir\n
          | 1 | 2 | 3 |
   Mano   -------------   Mano
          | 4 | 5 | 6 |
Derecha   -------------   Izquierda
...
"""
    ...
    while ubi != "" or len(ubications) == 0:
        ...
        if len(ubications) == 9:
            break
        ubi = input(
            """Si le duele en otra zona, indíquelo o pulse enter para salir\n
          | 1 | 2 | 3 |
          -------------
...
"""
    ...
    if len(newpercentages) == 0:
        ...
        print2(
            "Con los síntomas que tienes puede que tengas las siguientes enfermedades:"
        )
        ...
        sorted_res_final = dict(
            sorted(res_final.items(), key=lambda item: item[1], reverse=True)
        )
        ...
        if option in ["y", "Y"]:
            ...
            plt.bar(
                [x[:-1] for x in sorted_res_final],
                [sorted_res_final[x] for x in sorted_res_final],
                color=[
                    (
                        (x / max(sorted_res_final.values())),
                        1 - (x / max(sorted_res_final.values())),
                        0,
                        1,
                    )
                    for x in sorted_res_final.values()
                ],
            )
        ...
        # Mensaje final
        print2(
            "El chatbot ha finalizado, dirígase a la sala {} en la que se le harán las pruebas necesarias para confirmar las posibles patologías.\nSu número de cita es {}{}{}, espere a que salga en la pantalla de la sala de espera y será atendido por el médico {}.\n¡Muchas gracias por confiar en nuestro chatbot médico!".format(
                random.randint(1, 10),
                random.choice(letras),
                random.choice(letras),
                random.randint(1, 10),
                random.choice(apellidos).capitalize(),
            )
        )
